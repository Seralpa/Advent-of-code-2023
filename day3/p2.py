...
for i, line in enumerate(data):
	...
	for j, c in line_iter:
		if c.isdigit():
			...
			part = Part(int(line[j:j + n_len]), tuple(positions))
...
for i, line in enumerate(data):
	for j, c in enumerate(line):
		if c == "*":
			...
			around = [(i2, j2) for i2 in range(i2_start, i2_end) for j2 in range(j2_start, j2_end)]
			parts = set(pos2part[pos] for pos in around if data[pos[0]][pos[1]].isdigit())
			if len(parts) == 2:
